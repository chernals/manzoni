...
ProcIslands::ProcIslands(ParticlesSet* set, std::string xp) : 
    part_set(set)
{
...
ProcIslands::readParameters()
{
    ...
    // Read the mode
    if(XMLInputParser::getInstance().getAttributeTextFromPath(xpath, "mode") == "radial")
    {
	   radial = true;
    }
    else if(XMLInputParser::getInstance().getAttributeTextFromPath(xpath, "mode")=="square")
    {
	   radial=false;
    }
    else
    {
	   throw("Error islands mode");
    }
	...
	boxes_y = XMLInputParser::getInstance().getTextElementsFromPath(xpath + "/analysis/boxes/vertical");
      ...
      {
	boxes_x = std::vector<std::string>(1,"1");
	boxes_y = std::vector<std::string>(1,"1");	
      }
...
{    
  ...
  double min_y = -box_size+(2*box_size/b_y)*static_cast<int>((box_id-1)/b_x);
  double max_y = min_y + 2*box_size/b_y;
  double min_x = -box_size+(2*box_size/b_x)*((box_id-1)%b_x);
  ...
  std::cout << "Min y: " << min_y << " Max y: " << max_y << " Min x: " << min_x << " Max x: " << max_x << " Center P1: " << center_plane1 << " Center P2: " << center_plane2 << std::endl;
  ...
  for(int j=0;j<part_set->getParticlesNumber();++j)
    {
      if(radial)
	{
	  if((*store)(j,plane1)*(*store)(j,plane1)+(*store)(j,plane2)*(*store)(j,plane2) <= box_size*box_size)
	    {
