...
ParticlesSet::setTracking()
{
    ...
    if(XMLInputParser::getInstance().isFoundFromPath("./flows/alessandroFlow/initialDistribution/tracking"))
    {
        ...
        if(distr_dims == 4)
        {
            ...
            w = 0;
            for(std::vector<std::string>::iterator i = yp.begin(); i < yp.end(); i++)
            {
...
#ifdef DEBUG_FLAG
void
ParticlesSet::showStoreInfo(bool with_store)
{
    LOG(PARTICLES_SET, Informations about the store:);
    char* string = new char[BUFFER_SIZE];
    ...
    if(isSet)
    {
        LOG(PARTICLES_SET, .... The distribution is: set);
    }
    else
    {
        LOG(PARTICLES_SET, .... The distribution is: null);
    }
    std::string tmp_str = ".... Store type: " + distr_type;
    LOGV(PARTICLES_SET, tmp_str);
    delete [] string; 
    ...
    if(with_store)
    {
        LOG(PARTICLES_SET, .... Outputing the store:);
