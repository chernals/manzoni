...
class LaunchCampaign
   ...
   def sendJobs
     ...
     @input_processor.getPaths.each do |p|
        check_path = p + "islandsXXP.dat"
        if File.exist?(check_path)
          puts "Job already sent."
          else
        puts "Sending the job for the simulation: #{p} ..."
        job = BatchManager.new(p,@file_name,@queue, @debug)
        job.send
   ...
   def processMasterInput
    @input_processor = InputProcessor.new("campaigns/"+@campaign + "/" + @file_name)
     puts "Continue ?"
