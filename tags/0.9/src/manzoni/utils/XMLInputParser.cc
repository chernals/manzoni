...
{           
    ...
    try
    {
        ...
        this->simulation_name = xercesc::XMLString::transcode(sim_name);
				...
				const XMLCh* flow_name = rootElement->getAttribute(attr_flow_name);
				this->flow_name = xercesc::XMLString::transcode(flow_name);
    } // Try
...
XMLParser::getAttributeTextFromPath(std::string path, std::string attr)
{
    try
    {
	    ...
	    if(isFoundFromPath(path))
	    {
	        ...
	        int returned_length = attr_map->getLength();
