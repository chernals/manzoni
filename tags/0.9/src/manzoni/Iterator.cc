...
Iterator::iterate4d()
{
    ...
    for(t = 0; t < turns; t++)
    {
		...
		test_file << (*store)(m,X_INDEX) << " " << (*store)(m,XP_INDEX) << " " << (*store)(m,Y_INDEX) << " " << (*store)(m,YP_INDEX) << std::endl;	
							...
							iterateParticle4(tmp,kick_types[kick_id],chik,chik2,iota,omicron,x_cos,x_sin,y_cos,y_sin);
							...
							{		
							  ...
							  std::vector<std::vector<double> > orbit_left_1, orbit_left_2;
								...
								{
									invariant_left_1 += (orbit_left_1.at(i).at(0)-orbit_left_1.at(i-1).at(0))*0.5*(orbit_left_1.at(i-1).at(1)+orbit_left_1.at(i).at(1));
								}
								for(int i = 1; i < static_cast<int>(orbit_left_2.size()); i++)
								{
									invariant_left_2 += (orbit_left_2.at(i).at(0)-orbit_left_2.at(i-1).at(0))*0.5*(orbit_left_2.at(i-1).at(1)+orbit_left_2.at(i).at(1));
								}
								for(int i = 1; i < static_cast<int>(orbit_right_1.size()); i++)
								{
									invariant_right_1 += (orbit_right_1.at(i).at(0)-orbit_right_1.at(i-1).at(0))*0.5*(orbit_right_1.at(i-1).at(1)+orbit_right_1.at(i).at(1));
								}
								for(int i = 1; i < static_cast<int>(orbit_right_2.size()); i++)
								{
									invariant_right_2 += (orbit_right_2.at(i).at(0)-orbit_right_2.at(i-1).at(0))*0.5*(orbit_right_2.at(i-1).at(1)+orbit_right_2.at(i).at(1));
								}
...
Iterator::phaseRotation(double phase, int index)
{
    ...
    for(int m=0; m < particles_number; m++)
    {
	    ...
	    {
          	  if((*store)(m,0) >= 1.0 || (*store)(m,1) >= 1.0 || (*store)(m,0) <= -1.0 || (*store)(m,1) <= -1.0)
           	 {
               	 (*store)(m,0) = LOST;
               	 (*store)(m,1) = LOST;
           	 }
	    ...
	    {
          	  if((*store)(m,0) >= 1.0 || (*store)(m,1) >= 1.0 || (*store)(m,2) >= 1.0 || (*store)(m,3)>=1.0 || (*store)(m,0) <= -1.0 
				  || (*store)(m,1) <= -1.0 || (*store)(m,2) <= -1.0 || (*store)(m,3) <= -1.0)
           	 {
               	 (*store)(m,0) = LOST;
               	 (*store)(m,1) = LOST;
               	 (*store)(m,2) = LOST;
...
Iterator::readParameters()
{
    ...
    if(dimensions == TWO_DIMENSIONS || dimensions == FOUR_DIMENSIONS || dimensions == SIX_DIMENSIONS)
    {
        ...
        for(int i=0; i < n_kicks_; i++)
        {
            ...
            // Is there a damper ?
            #ifdef DAMPER_FLAG
            	std::string tmp_path = "./flows/henonFlow/iterator/kick" + i2s(i) + "/damper";
  ...
  kick_types.resize(n_kicks_);
    ...
    for(int i=0; i < n_kicks_; i++)
    {
        ...
        if(XMLInputParser::getInstance().isFoundFromPath(path))
        {
            ...
            if(kick_types[i] != static_cast<std::string>("SO") && 
               kick_types[i] != static_cast<std::string>("S") && 
               kick_types[i] != static_cast<std::string>("O") && 
               kick_types[i] != static_cast<std::string>("L") &&
               kick_types[i] != static_cast<std::string>("U4") &&
							kick_types[i] != static_cast<std::string>("SK") )
            {
                throw("Invalid kick type !");
            }
