...
Iterator::iterate4d()
{
    ...
    for(t = 0; t < turns; t++)
    {
	    ...
	    if(t == 15000)
	    {
			for(int m=0;m < particles_number;m++)
			{
				if((*store)(m,X_INDEX) > 0.3)
				(*store)(m,XP_INDEX) += 0.1;
			}
	    }*/
...
Iterator::readParameters()
{
    ...
    if(part_set->getDims() == TWO_DIMENSIONS || part_set->getDims() == FOUR_DIMENSIONS || part_set->getDims() == SIX_DIMENSIONS)
    {
	    ...
	    else if(kick_types[i] == "SOP" || kick_types[i] == "OP")
	    {
		beta_x[i] = XMLInputParser::getInstance().getFirstTextd(kick_path + "/beta_x");
	    }
	    ...
	    if(kick_types[i] == "SOP")
	    {
		param_evolution->readEvolvingVariable(i,static_cast<std::string>("K2"),K2);
	    }
	    if(kick_types[i] == "SOP" || kick_types[i] == "OP")
	    {
		param_evolution->readEvolvingVariable(i,static_cast<std::string>("K3"),K3);
	    }
            ...
            // Damper
            	...
            	if(isDamperSet[i])
            	{
                    ...
                    {
                        throw("Damper  must be a windowed curve !");
                    }
                	#ifdef DEBUG_FLAG
    ...
    if(part_set->getDims() == 4 || part_set->getDims() == 6)
    {
	    ...
	    if(kick_types[i] == "SOP" || kick_types[i] == "OP")
	          {
	       beta_y[i] = XMLInputParser::getInstance().getFirstTextd(kick_path + "/beta_y");
	         }
...
Iterator::createContainers()
{
	...
	if(part_set->getDims() == TWO_DIMENSIONS || part_set->getDims() == FOUR_DIMENSIONS || part_set->getDims() == SIX_DIMENSIONS)
	{
	    ...
	    K2 = new CurveParameters[n_kicks_];
	    K3 = new CurveParameters[n_kicks_];
	    normalizations.resize(n_kicks_);
	    beta_x.resize(n_kicks_);
	    #ifdef DAMPER_FLAG
  ...
  kick_types.resize(n_kicks_);
    ...
    for(int i=0; i < n_kicks_; i++)
    {
        ...
        if(XMLInputParser::getInstance().isFoundFromPath(path))
        {
            ...
            if(kick_types[i] != static_cast<std::string>("SO") && kick_types[i] != static_cast<std::string>("S") && kick_types[i] != static_cast<std::string>("O") && kick_types[i] != static_cast<std::string>("L") && kick_types[i] != static_cast<std::string>("SOP") && kick_types[i] != static_cast<std::string>("OP"))
            {
                throw("Invalid kick type !");
            }
