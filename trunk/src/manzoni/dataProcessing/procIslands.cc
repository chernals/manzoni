          ...
          i_tmp++;
           if(s2n<int>(*it) > s2n<int>(boxes_x.at(i)) * s2n<int>(boxes_y.at(i)))
            ...
            else
            {
              ...
              int count_tmp = processBox(tmp);
                if(i_tmp == box_core)
                  {
              trapping = static_cast<double>(count_tmp)/static_cast<double>(part_set->getParticlesNumber());
                }
            }
        } // Islands
}
else
{
    ...
    std::cout << std::setw(SPACING) << "Trapping:" << std::setw(SPACING) << trapping << std::endl;
	#endif
	file << "Total:" << std::setw(SPACING) << part_set->getParticlesNumber() 
	     << std::endl;
	file << "Losses:" << std::setw(SPACING) << losses << std::endl;
  file << "Trapping:" << std::setw(SPACING) << trapping << std::endl;
	
	// Close the file
  file.close();
  
  // Write trapping into a file
  std::ofstream trapping_file("trapping.out");
  trapping_file << XMLInputParser::getInstance().getAttributeTextFromPath(".", "id") << " " << trapping;
...
ProcIslands::readParameters()
{
    ...
    n_processings = XMLInputParser::getInstance().howManyFoundFromPath(xpath + "/analysis");
      ...
      // Core box
      box_core = s2n<int>(XMLInputParser::getInstance().getTextElementsFromPath(xpath + "/analysis/boxes/core").at(0));
