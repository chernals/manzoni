...
{           
    ...
    try // Instanciate the MathLink singleton
    {
      ML::getInstance();
    }
    catch(const char* excep)
    {
      ERRORV(MANZONI ML CATCH, excep);
      ERROR(MANZONI ML CATCH, Forwarding...);
      throw("Exception catched in MANZONI MATHLINK instanciation"); 
    }
    
	// Run the flow
	flow->run();
	
  // Finished properly, save status in a file
  std::ofstream tmp_file("run");
  tmp_file << "Successfully";
