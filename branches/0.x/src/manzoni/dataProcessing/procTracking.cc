...
ProcTracking::ProcTracking(ParticlesSet* p, std::string xp) : 
    spacing(13),
    precision(5)
{
    // Initialization
    ...
    if(XMLInputParser::getInstance().isFoundFromPath("./flows/alessandroFlow/dataProcessing/final/tracking/coherent") && part_set->isCoherent())
    {
        withCoherent = true;
    ...
    if(XMLInputParser::getInstance().isFoundFromPath("./flows/alessandroFlow/dataProcessing/final/tracking/particles") && part_set->isTracking())
    {
        withParticles = true;
...
ProcTracking::readParameters()
{
    ...
    if(withCoherent)
    {
        file_name_coherent = XMLInputParser::getInstance().getAttributeTextFromPath("./flows/alessandroFlow/dataProcessing/final/tracking/coherent","file");
    }
    // Particles
    if(withParticles)
    {
        file_name_particles = XMLInputParser::getInstance().getAttributeTextFromPath("./flows/alessandroFlow/dataProcessing/final/tracking/particles","files");
    }
    
    // General
    withFFT = (XMLInputParser::getInstance().getAttributeTextFromPath("./flows/alessandroFlow/dataProcessing/final/tracking","fft") == "yes");
    withPlots = (XMLInputParser::getInstance().getAttributeTextFromPath("./flows/alessandroFlow/dataProcessing/final/tracking","plots") == "yes");
    if(withFFT)
    {
        withTune = XMLInputParser::getInstance().isFoundFromPath("./flows/alessandroFlow/dataProcessing/final/tracking/fft/tune");
    }
    else
    {
        withTune = false;
...
ProcTracking::process()
{
    ...
    {
        fft_window = XMLInputParser::getInstance().getFirstTexti("./flows/alessandroFlow/dataProcessing/final/tracking/fft/window");
        fft_step = XMLInputParser::getInstance().getFirstTexti("./flows/alessandroFlow/dataProcessing/final/tracking/fft/step");
        fft_out_step = XMLInputParser::getInstance().getFirstTexti("./flows/alessandroFlow/dataProcessing/final/tracking/fft/stepOut");
        if(turns > fft_window)
        {
...
ProcTracking::computeFFT(int dimension)
{
    ...
    if(withCoherent)
    {
        ...
        if(dimension == X_PLANE)
        {
            file_tune_coherent.open((XMLInputParser::getInstance().getAttributeTextFromPath("./flows/alessandroFlow/dataProcessing/final/tracking/fft/tune","file")+"CoherentHorizontal.dat").c_str());
        }
        else if(dimension == Y_PLANE)
        {
            file_tune_coherent.open((XMLInputParser::getInstance().getAttributeTextFromPath("./flows/alessandroFlow/dataProcessing/final/tracking/fft/tune","file")+"CoherentVertical.dat").c_str());
        }
    }
...
ProcTracking::write2File()
{
    ...
    if(withParticles)
    {
	    LOG(PROC TRACKING,Number of tracked particles:);
		   LOGV(PROC TRACKING, i2s(ics_count)); 
        for(int i = 0; i < ics_count; i++)
        {
            file = file_name_particles + i2s(i+1) + ".dat";
    	std::ofstream f;
            f.open(file.c_str());
	    LOG(PROC TRACKING, Logging in file ...);
	    LOGV(PROC TRACKING, file.c_str());
